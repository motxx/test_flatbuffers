...
SampleRoot::UnPackTo(SampleRootT *_o,
  ...
  {
    const flatbuffers::Vector<uint8_t>* _e = objects_type();
    if (_e) {
      _o->objects_type.resize(_e->size());
      for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) {
        _o->objects_type[_i] = (Object)_e->Get(_i);
      }
    }
  };

  {
    const flatbuffers::Vector<flatbuffers::Offset<void>>* _e = objects();
    if (_e) {
      _o->objects.resize(_e->size());
      for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) {
        _o->objects[_i].Set(_e->Get(_i));
      }
    }
  };
