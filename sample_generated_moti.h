...
// union Hoge { ... } -> [Hoge]Union
struct ObjectUnion {
  // Unionは2つの値のみで構成。
  // type: enum, talbe: *NativeTable 

  // enum Hoge { ... }
  Object type;
  // unionなので内容は一つのポインタになる。NativeTableは単なるentityで、中身は空。
  ...
  // tableがポインタなため、shallow copyとなるのでcopy ctorは非対応
  ObjectUnion(const ObjectUnion &);
  // 同様にcopy assignment不可
  ObjectUnion &operator=(const ObjectUnion &);

  // tableはRAIIで管理されているので、tableを各型にreinterpret_castした上でdeleteする。
  ~ObjectUnion() { Reset(); }

  // tableのdelete処理。C++の場合、delete nullptrは許可されている。
  void Reset();

  // unionに各型をassignする方法。引数はuniversal referenceとなる。
  template <typename T>
  void Set(T&& value) {
    // 現状を消す
    Reset();
